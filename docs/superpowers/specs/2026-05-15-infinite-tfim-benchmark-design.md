...
- Vovrosh et al., "Simulating dynamics of the two-dimensional transverse-field Ising model: a comparative study of large-scale classical numerics", arXiv:2511.19340 (2025), https://arxiv.org/abs/2511.19340. This is finite-system cross-method dynamics context and should be secondary for the infinite benchmark.

## Architecture

Add a narrow star-model abstraction:
...
struct TFIMStarModel <: AbstractStarModel
    J::Float64
    h::Float64
end
```
...
```julia
star_hamiltonian(model)::Matrix{ComplexF64}
star_gate(model, step; evolution = :real)::Matrix{ComplexF64}
...
Extend the existing update path without duplicating it:
...
Extend `TrotterParams` so the model travels with the evolution configuration:

```julia
TrotterParams(dt, order, evolution, model, maxdim, cutoff)
```

The current constructor shape should remain supported for PXP compatibility, either by an overload or by a compatibility shim that maps the old `projected::Bool` argument to `PXPStarModel(projected)`.
...
tfim_energy_density_simple(psi, model::TFIMStarModel)::Float64
measure_tfim_simple(psi, model::TFIMStarModel)::TFIMObservableSummary
```

`tfim_energy_density_simple` should average the same `star_hamiltonian(model)` over all unit-cell representatives through the existing star-patch expectation logic. `nearest_neighbor_zz_simple` should support canonical directions `:right` and `:up`, with the same direction validation conventions already used by PXP diagnostics.
...
- large field ratio, such as `h/J = 5.0`
...
- TFIM simple observable summary
...
`BenchmarkResult` should be a reproducibility record, not just a table row. It should include:

- benchmark name
- timestamp or run label supplied by caller
...
JSON is the canonical output for full metadata. CSV is for sweep tables.
...
1. Existing PXP tests pass unchanged.
2. `PXPStarModel` reproduces current PXP gates exactly.
3. TFIM dense-gate tests pass.
4. TFIM product-state observable tests pass.
5. A short infinite TFIM run completes deterministically on a `10x10` cell.
6. The benchmark runner writes JSON and CSV records containing full reproducibility metadata.
7. Documentation clearly states that simple-update results are regression diagnostics and not CTM-quality physics claims.
...
- Put benchmark runner types and file writers in a new `src/Benchmarks.jl` module. Export `BenchmarkSpec`, `BenchmarkResult`, `run_benchmark`, `write_benchmark_json`, and `write_benchmark_csv` only after their tests are in place.
- Do not add a JSON dependency in the first pass. Implement a tiny local JSON writer for the scalar, string, boolean, vector, and dictionary-like data used by `BenchmarkResult`; replace it later only if the result schema grows beyond that.
