...
function square_star_basis_allowed(bits)
    length(bits) == SQUARE_STAR_SITES || throw(ArgumentError("square star basis must have 5 sites"))
...
function square_star_blockade_projector()
    ...
    for i in 1:dim
        projector[i, i] = square_star_basis_allowed(_basis_bits(i, SQUARE_STAR_SITES)) ? 1 : 0
    end
    return projector
end
